...
class DevBoxVM
  def self.init
    DevBoxConf.config.vm.define DevBoxConf.settings('name', 'debian-devbox')
    ...
    DevBoxConf.config.vm.boot_timeout     = DevBoxConf.settings('boot_timeout', 120)
    ...
    DevBoxConf.config.vm.provider "virtualbox" do |vb|
      ...
      vb.memory = DevBoxConf.settings('memory', 1024)
    end
